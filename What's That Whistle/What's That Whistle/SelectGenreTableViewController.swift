...
class SelectGenreTableViewController: UITableViewController {
    ...
    override func viewDidLoad() {
        ...
        self.navigationItem.backBarButtonItem = UIBarButtonItem(title: "Genre", style: .Plain, target: nil, action: nil)
    ...
    override func tableView(tableView: UITableView, didSelectRowAtIndexPath indexPath: NSIndexPath) {
        if let cell = self.tableView.cellForRowAtIndexPath(indexPath) {
            let genre = cell.textLabel?.text ?? SelectGenreTableViewController.genres.first
            let addCommentsVC = AddCommentsViewController()
            addCommentsVC.genre = genre
            self.navigationController?.pushViewController(addCommentsVC, animated: true)
        }
    }
